...
const mapDispatchToProps: MapDispatch<MappedProps> = (
  ...
  initGame: (name: string, money: number, opponents: number) =>
    dispatch({
    ...
    return (
      <Fragment>
        <div className={Styles.Intro}>
          <h1>NARDIS</h1>
          <h3>A TURN-BASED TRADING GAME</h3>
        </div>
        <div className={Styles.CreateGame}>
          <form onSubmit={this.initGame}>
            {Object.keys(this.state.base).map(
              (key: string): JSX.Element => {
                const entry: CreateGameInputConfig = this.state.base[key];
                return (
                  <Input
                    key={key}
                    changed={(event: ChangedEventElement) =>
